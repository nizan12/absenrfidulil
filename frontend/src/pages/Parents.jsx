...
import { Plus, Search, Edit2, Trash2, Loader2, UserPlus, Phone, LayoutGrid, List, X, Camera } from 'lucide-react';
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
                        <UserPlus className="text-primary-600" />Data Orang Tua
                    </h1>
                                ...
                                {parents.length > 0 ? parents.map((p) => (
                                    <tr key={p.id}>
                                        ...
                                        <td>
                                            <div className="flex items-center gap-3">
                                                {p.photo ? (
                                                    <img src={`${import.meta.env.VITE_API_URL?.replace('/api', '') || 'http://localhost:8000'}/storage/${p.photo}`} alt={p.name} className="w-10 h-10 rounded-full object-cover" />
                                                ) : (
                                        ...
                                        <td><span className="flex items-center gap-1"><Phone size={14} />{p.phone}</span></td>
                                        <td className="capitalize">{p.relationship}</td>
                    ...
                    {pagination && pagination.lastPage > 1 && (
                        <div className="flex flex-col sm:flex-row items-center justify-between p-4 border-t border-gray-200 gap-4">
                            ...
                            <div className="flex gap-2">
                                <button
                                    onClick={() => fetchParents(pagination.currentPage - 1)}
